...
### FreePBX BMO (Big Module Object)

FreePBX 17 loads module hooks through a BMO class named after the module (`Zerotouchsip.class.php`, class `FreePBX\modules\Zerotouchsip`). Procedural helpers remain in `functions.inc.php` for legacy call sites and Asterisk dialplan hooks.

| Concern | Location | Notes |
|---|---|---|
| Module entry (BMO) | `Zerotouchsip.class.php` | `doConfigPageInit()`, `getActionBar()`, `backup()` / `restore()` stubs |
| Admin page | `page.zerotouchsip.php` | Tab shell; dispatches to `views/` via controllers |
| Legacy helpers | `functions.inc.php` | Phone CRUD, SQL helpers, `zerotouchsip_get_config()` for Asterisk NOTIFY |
| Admin CSS | `assets/css/zerotouchsip.css` | Auto-loaded by `framework_include_css()` from `config.php` |
| Admin JS | `assets/js/*.js` | Auto-loaded by `framework_include_js()`; views keep only inline init scripts that depend on PHP data |
| Backup / restore | `Backup.php`, `Restore.php` | Modern FreePBX 17 pipeline (BMO `backup()` / `restore()` are no-op stubs) |

**Hook migration (legacy → BMO)**

- `zerotouchsip_configpageinit` / `configpageload` → `Zerotouchsip::doConfigPageInit()` (e.g. “Edit in ZeroTouchSIP” links on Extensions)
- `getActionBar` → `Zerotouchsip::getActionBar()` (hides default Submit/Reset on module pages)
- `zerotouchsip_get_config($engine)` stays in `functions.inc.php` — not `BMO::getConfig()`, which is reserved for DB settings in `FreePBX_Helpers`

**Assets**

`install.php` symlinks `assets/` to `/admin/assets/zerotouchsip/`. The framework scans `assets/css/` and `assets/js/` and emits `<link>` / `<script>` tags; individual views must not duplicate `<script src="...">` for module bundles.
...
```text
zerotouchsip/
├── assets/                         # JS/CSS for FreePBX admin UI (symlinked to /admin/assets/zerotouchsip/)
│   ├── css/
│   │   └── zerotouchsip.css        # Module styles (framework_include_css)
│   └── js/
...
│   └── fanvil/
│   └── yealink/
├── views/                          # FreePBX admin views (no inline CSS; minimal inline JS)
│   ├── zts_phones.php
│   ├── zts_phones_edit.php
│   ├── zts_networks.php
│   ├── zts_networks_edit.php
│   └── zts_general.php
├── Zerotouchsip.class.php          # BMO entry (hooks, page init)
├── Backup.php
├── Restore.php
├── functions.inc.php               # Legacy helpers + zerotouchsip_get_config()
├── install.php
├── upgrade.php
├── uninstall.php
├── module.xml
...
| BMO class | `.../zerotouchsip/Zerotouchsip.class.php` |
| Admin CSS | `/admin/assets/zerotouchsip/css/zerotouchsip.css` |
| Admin JS | `/admin/assets/zerotouchsip/js/*.js` |
...
- **Architecture**: BMO class (`Zerotouchsip.class.php`) + modular PHP services for FreePBX 17
