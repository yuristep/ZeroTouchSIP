...
class Zts_NetworkWifiProfileService
{
	...
	public static function yealinkConfigLines(array $networkSettings, array $deviceSettings = array())
	{
		...
		foreach ($profiles as $profile)
		{
			...
			$lines[] = sprintf('static.wifi.%d.label = %s', $idx, $label);
			$lines[] = sprintf('static.wifi.%d.ssid = %s', $idx, $profile['ssid']);
			$lines[] = sprintf('static.wifi.%d.password = %s', $idx, $profile['password']);
